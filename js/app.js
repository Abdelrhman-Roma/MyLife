// MYLIFE — main application logic
// Handles auth, routing, data persistence, and page rendering for all standard pages.
// The gym page has its own dedicated gym.js and is excluded from this file's rendering.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  else if (currentPage === 'gym') {
    // gym.js takes over completely; app.js only applies the theme
...
function renderForm(pageKey) {
  ...
  // Guard: these elements don't exist on the gym page (handled by gym.js)
    ...
    entryForm.innerHTML = `
      <div class="form-grid">
        ...
        <label>Accent palette
          <select name="palette">
            <option value="palette-1" ${selected(s.palette, 'palette-1')}>Palette 1 — Blue / Red / Yellow / Beige</option>
            <option value="palette-2" ${selected(s.palette, 'palette-2')}>Palette 2 — Yellow / Green</option>
            <option value="palette-3" ${selected(s.palette, 'palette-3')}>Palette 3 — Teal / Green</option>
            <option value="palette-4" ${selected(s.palette, 'palette-4')}>Palette 4 — Nature / Sage</option>
            <option value="palette-5" ${selected(s.palette, 'palette-5')}>Palette 5 — Brown / Orange</option>
            <option value="palette-6" ${selected(s.palette, 'palette-6')}>Palette 6 — Dark Blue / Red</option>
            <option value="palette-7" ${selected(s.palette, 'palette-7')}>Palette 7 — Black / Teal / Purple</option>
            <option value="palette-8" ${selected(s.palette, 'palette-8')}>Palette 8 — Wine / Green / Cream</option>
            <option value="palette-9" ${selected(s.palette, 'palette-9')}>Palette 9 — Red / Yellow</option>
            <option value="palette-10" ${selected(s.palette, 'palette-10')}>Palette 10 — Black / Blue / Orange</option>
            <option value="palette-11" ${selected(s.palette, 'palette-11')}>Palette 11 — Purple / Pink / Peach</option>
            <option value="palette-12" ${selected(s.palette, 'palette-12')}>Palette 12 — Blue Gradient</option>
            <option value="palette-13" ${selected(s.palette, 'palette-13')}>Palette 13 — Soft Green</option>
            <option value="palette-14" ${selected(s.palette, 'palette-14')}>Palette 14 — Yellow / Green</option>
            <option value="palette-15" ${selected(s.palette, 'palette-15')}>Palette 15 — Navy / Beige / Brown</option>
            <option value="palette-16" ${selected(s.palette, 'palette-16')}>Palette 16 — Nature Green</option>
            <option value="palette-17" ${selected(s.palette, 'palette-17')}>Palette 17 — Cream / Red</option>
          </select>
        </label>
...
function renderList(pageKey) {
  ...
  if (!listTitle) return; // gym page has its own layout
  ...
  if (pageKey === 'settings') {
    ...
    byId('data-list').innerHTML = `
      <article class="data-card stacked">
        ...
        <p>Calories: ${s.calorieTarget} kcal — Protein: ${s.proteinTarget}g — Carbs: ${s.carbTarget}g — Fat: ${s.fatTarget}g</p>
      </article>
      <article class="data-card stacked">
...
function renderChecklist(pageKey) {
  ...
  byId('data-list').innerHTML = items.map((item) => `
    <article class="data-card checklist-card ${item.completed ? 'complete' : ''}">
      ...
      <p>${page.labels.map((k) => item[k] !== undefined ? `${labelize(k)}: ${escapeHtml(String(item[k]))}` : '').filter(Boolean).join(' · ')}</p>
